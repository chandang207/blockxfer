...
contract PhoneTransfer is Ownable, AxelarExecutable 
{
    ...
    function transferTokens(address to, address tokenAddr, uint256 amount) public 
    {
        ...
        //Limiting normal transfers to showing only successful transactions(Will check the feasibility of showing failed transactions only if time permits)
        /*emit prePaymentEve
        (
    ...
    function sendEcrowPayment(string memory unregisteredReceiver, address token, uint amount) public
    {
    ...
    function resolvePhonetoPublicAddress(uint transactionNo, string memory unregisteredReceiver, address updatePubKeyOfUnregisteredReceiver) public onlyOwner
    {
        ...
        require(payment.receiver == address(0), "Escrow payment ownership is already changed");
        bytes32 unregisteredReceiverHash = keccak256(abi.encodePacked(unregisteredReceiver));
        bytes32 paymentUnregisteredReceiverHash = keccak256(abi.encodePacked(payment.unregisteredReceiver));

        require(unregisteredReceiverHash == paymentUnregisteredReceiverHash, "Receiver doesn't match");
