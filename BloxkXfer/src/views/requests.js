    ...
    async function fetchPhoneNumbers(transactions) {
      for (const transaction of transactions) {
        if (!phoneNumbers[transaction.requestedBy]) {
          const phoneNumber = await getPhoneNumberFromPubKey(transaction.requestedBy);
        ...
        if (!phoneNumbers[transaction.requestedFrom]) {
          const phoneNumber = await getPhoneNumberFromPubKey(transaction.requestedFrom);
          setPhoneNumbers((prevPhoneNumbers) => ({
            ...prevPhoneNumbers,
    ...
    const getEncryptedPhNum = async () => 
    {
      let encryptedStr;
        try {
          ...
          console.log(`auth:    ${removeSpaces(auth.mobileNumber)}`)
          const response = await fetch(`${baseUrl}/api/encrypt/${removeSpaces(auth.mobileNumber)}`, {
            method: 'GET',
            headers: {
    ...
    useEffect(() => {
        const fetchRequestsData = async () => {
          try {
            ...
            const baseUrl = 'http://localhost:3000';
            const response = await axios.get(`${baseUrl}/api/requests/${signer._address}`);
      ...
      const initiateAcceptRequest = async ( amount, transactionNo, requestedTokenAddr, status) => {
        console.log("Initiating acceptance of request - 1");
        ...
        console.log(signer._address)
        const phoneContract = new ethers.Contract(deployedPhoneContractAddress, phoneContractABI, signer);
        const tokenContract = new ethers.Contract(requestedTokenAddr, tokenContractABI, signer);
        const tokens = ethers.utils.parseUnits(amount, 18);

        //console.log(ethers.utils.formatUnits(await tokenContract.allowance(await signer.getAddress(), '0xBBA13a375b2E9a4d79ed8A8f0FB2313A335B463c'), 6));
        await acceptRequest(phoneContract, tokenContract, transactionNo, tokens, status)
      };
      ...
      const cliamHelperUtility = async (transactionNo, currentSigner) => 
      {
        try{
          ...
          const gasPrice = ethers.utils.parseUnits("5", "gwei");
          const gasLimit = 500000;
          const adminPhoneContract = new ethers.Contract(deployedPhoneContractAddress, phoneContractABI, connectedWallet);
          let encryptedValue = await getEncryptedPhNum();
          const tx = await adminPhoneContract.resolvePhonetoPublicAddress(transactionNo, encryptedValue,currentSigner,
            {
              gasPrice: gasPrice,
              gasLimit: gasLimit,
            });
          await tx.wait();
          console.log(tx);
          console.log('Admin call is done');
        }
        catch(error)
        {
          console.log(error);
  ...
  return (
    ...
    {requestsData && escrowData ?
      (<Table>
        ...
        <tbody>          
          {requestsData ? requestsData.sentRequests.pending.map((request) => 
          (
                <tr key={request.transactionNo}>
                    ...
                    >
                        ...
                        href="#pablo"
                        onClick={() => initiateCancelRequest(request.transactionNo, 0)}
                        variant="danger"
                        >
          ...
          {requestsData ? requestsData.sentRequests.fullfilled.map((request) => 
          (
                <tr className="success" key={request.transactionNo}>
                    ...
                    <td>{tokenAddresses[request.token]}</td>
                    <td>{request.amount}</td>
                    <td><td><i class="fa fa-ban" aria-hidden="true"></i></td></td>
                </tr>
            ))
          : "Loading"}
          
          {requestsData ? requestsData.receivedRequests.pending.map((request) => 
          (
                <tr key={request.transactionNo}>
                    ...
                    <OverlayTrigger
                        href="#pablo"
                        onClick={(e) => e.preventDefault()}
                        overlay={<Tooltip id="tooltip-255158527">Accept Request</Tooltip>}
                    >
                        <Button
                        className="btn-link btn-xs"
                        href="#pablo"
                        onClick={() => initiateAcceptRequest(request.amount, request.transactionNo, request.token, 1)}
                        variant="success"
                        >
                    ...
                    <OverlayTrigger
                        href="#pablo"
                        onClick={(e) => e.preventDefault()}
                        overlay={<Tooltip id="tooltip-255158527">Reject Request</Tooltip>}
                    >
                        <Button
                        className="btn-link btn-xs"
                        href="#pablo"
          ...
          {requestsData ? requestsData.receivedRequests.fullfilled.map((request) => 
          (
                <tr className="success" key={request.transactionNo}>
                    <td style={{width:'2px'}}><FontAwesomeIcon icon={faPaperPlane} style={{ color: 'red' }} className="mr-4 sent-trans" /></td>
                    <td className="text-center">{request.transactionNo}</td>
          ...
          {escrowData ? escrowData.receivedEscrow.pending.map((escrowRequest) => 
          (
                <tr key={escrowRequest.transactionNo}>
                    ...
                    <OverlayTrigger
                        href="#pablo"
                        onClick={(e) => e.preventDefault()}
                        overlay={<Tooltip id="tooltip-255158527">Claim funds</Tooltip>}
                    >
                        <Button
                        className="btn-link btn-xs"
                        href="#pablo"
                        onClick={() => initiateClaimRequest(escrowRequest.transactionNo, 3)}
                        variant="success"
                        >
                         <i class="fa fa-check" aria-hidden="true" style={{color:"green"}}></i>
                        </Button>
          ...
          {escrowData ? escrowData.receivedEscrow.fullfilled.map((escrowRequest) => 
          (
